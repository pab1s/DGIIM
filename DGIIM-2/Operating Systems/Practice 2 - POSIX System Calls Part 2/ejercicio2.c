...
int main(int argc, char *argv[]) {
    ...
    nptr to a long integer value according to the given base, which must be  be‐
    ...
    while ((ed = readdir(dir)) != 0) {
        ...
        */
        ...
        if (S_ISREG(atributos.st_mode)) {
            ...
            printf("%s: %o ", cadena2,
                   atributos.st_mode);  // %o formateo a octal
            ...
            if (chmod(cadena, permisos) < 0) {
                ...
                /* The strerror() function returns a pointer to a  string  that
                   describes the  error  code  passed  in  the  argument  errnum
                           ...
                           The  value  in  errno  is significant only when the
