...
int buscar(const char* path, const struct stat* atributes, int flags,
           struct FTW* ftw);
...
int buscar(const char* path, const struct stat* atributes, int flags,
           struct FTW* ftw) {
    for (int i = 0; i < ftw->level; i++) printf("\t");
    ...
    if ((atributes->st_mode & S_IXOTH) == S_IXOTH &&
        (atributes->st_mode & S_IXGRP) == S_IXGRP) {
        contador++;
